use core::{
    ...
    clone::Clone,
    mem,
    result::Result::Ok, convert::{TryInto, AsRef}, marker::PhantomData, default::Default,
};

use crate::{clock, thread, file::{RrosFileBinding, RrosFile, rros_open_file}, Rros, xbuf, proxy, control::{self, ControlOps}};
use alloc::rc::Rc;
...
impl RrosElementfpriv {
    fn new() -> Self {
        Self { filp: None, efd: file::FileDescriptorReservation{fd: 620} }
    }
}
...
fn create_element_device(
    ...
    let res = do_element_visibility(e.clone(), fac, &mut rdev);
    let e_clone = e.clone();
    let mut e_mut = e_clone.borrow_mut();
    let filp = e_mut.fpriv.filp.as_mut().unwrap().get_ptr();
    pr_info!("the address of filp location 7 is {:p}, {:p}", filp, &filp);
    ...
    // }
    unsafe { bindings::fd_install(e_mut.fpriv.efd.reserved_fd(), filp) };
    pr_info!("the address of filp location 8 is {:p}, {:p}", filp, &filp);
	// 	e->refs++;
...
// 	 */
    ...
    let inner = unsafe{reg.unwrap().inner.as_ref()};
    let cdev = unsafe{(inner.unwrap().cdevs[0]).as_ref()};
    let ops = unsafe{(*(cdev.unwrap().0)).ops};
    let filp = unsafe{bindings::anon_inode_getfile(
    ...
    pr_info!("the address of filp location 1 is {:p}, {:p}", filp, &filp);
// /*
// * Create a private user element, passing the real fops so
// * that FMODE_CAN_READ/WRITE are set accordingly by the vfs.
// */
...
// */
    ...
    let ret = bind_file_to_element(filp, e.clone());
    pr_info!("the address of filp location 4 is {:p}, {:p}", filp, &filp);
// if (ret) {
...
// replace_fops(filp, fac->fops);
    ...
    if (efd.reserved_fd() < 0) {
        panic!("get_unused_fd_flags failed");
        // filp_close(filp, current->files);
        // ret = efd;
        // return ret;
    }

    pr_info!("the address of filp location 5 is {:p}, {:p}", filp, &filp);
    e_mut.fpriv.filp = Some(File{ptr: filp});
    pr_info!("the address of filp location 6 is {:p}, {:p}", e_mut.fpriv.filp.as_ref().unwrap().ptr , &(&(e_mut).fpriv.filp.as_ref().unwrap().ptr));
    pr_info!("efd: {}", efd.reserved_fd());
    e_mut.fpriv.efd = efd;
    // (e_borrow.clone_flags & RROS_CLONE_COREDEV) == RROS_CLONE_COREDEV;
...
// 		return -ENOMEM;
    ...
    pr_info!("the address of filp location 2 is {:p}, {:p}", filp, &filp);
    let ret = fbind.rfile.borrow_mut().rros_open_file(filp)?;
    pr_info!("the address of filp location 3 is {:p}, {:p}", filp, &filp);
    pr_info!("the address of fbind.rfile.filp.oob_data is {:p}", fbind.rfile.as_ptr());
...
// 	fbind->element = e;
    let mut fbind: Box<RrosFileBinding> = Box::try_new(fbind)?;
    ...
    pr_info!("the address of fbind_ptr.rfile.filp.oob_data is {:p}", (*((*fbind_ptr).rfile.borrow_mut().filp)).oob_data);
    pr_info!("the address of filp.oob_data is {:p}", (*fbind_ptr).rfile.as_ptr());

//    filp.set_private_data(fbind_ptr as *mut c_types::c_void);
    unsafe{(*filp).private_data = fbind_ptr as *mut c_types::c_void;}
    ...
    let res = match fac_lock.inside {
        Some(ref mut inside) => {
            ...
            if flags == 0 {
                ...
                match name.to_str() {
                    Ok("RROS_THREAD_DEV") => {
                        let mut ele_chrdev_reg: Pin<Box<chrdev::Registration<{thread::CONFIG_RROS_NR_THREADS}>>> = chrdev::Registration::new_pinned(name, 0, this_module)?;
                        inside.register = Some(ele_chrdev_reg);
                        inside.register.as_mut().unwrap().as_mut().register::<thread::ThreadOps>()?;
                    },
                    Ok("RROS_XBUF_DEV") => {
                        let mut ele_chrdev_reg: Pin<Box<chrdev::Registration<{xbuf::CONFIG_RROS_NR_XBUFS}>>> = chrdev::Registration::new_pinned(name, 0, this_module)?;
                        inside.register = Some(ele_chrdev_reg);
                        inside.register.as_mut().unwrap().as_mut().register::<xbuf::XbufOps>()?;
                    },
                    Ok("RROS_PROXY_DEV") => {
                        let mut ele_chrdev_reg: Pin<Box<chrdev::Registration<{proxy::CONFIG_RROS_NR_PROXIES}>>> = chrdev::Registration::new_pinned(name, 0, this_module)?;
                        inside.register = Some(ele_chrdev_reg);
                        inside.register.as_mut().unwrap().as_mut().register::<proxy::ProxyOps>()?;
                    }
                    Ok(_) => {
...
fn create_core_factories(
    ...
    for i in 0..nr {
        let mut ret = rros_create_factory(
            &mut factories[i],
            i,
            rros_class.clone(),
            chrdev_reg,
...
// return -EINVAL;
    ...
    pr_info!("the u_attrs is {:p}", u_attrs);
// u_attrs = rros_valptr64(req.attrs_ptr, void);
    ...
    pr_info!("the value is {:?} ", cstr_u_name[0]);
    pr_info!("the value is {:?} ", cstr_u_name[0]);
    let e: Rc<RefCell<RrosElement>> = if cstr_u_name[0] == 88 {
        pr_info!("x");        
        unsafe{(*xbuf::RROS_XBUF_FACTORY.locked_data().get()).build.unwrap()(&mut xbuf::RROS_XBUF_FACTORY, cstr_u_name, Some(u_attrs), real_req.clone_flags.try_into().unwrap(), &state_offset)}
    } else if cstr_u_name[0] == 80 {
        pr_info!("p");        
        unsafe{(*proxy::RROS_PROXY_FACTORY.locked_data().get()).build.unwrap()(&mut proxy::RROS_PROXY_FACTORY, cstr_u_name, Some(u_attrs), real_req.clone_flags.try_into().unwrap(), &state_offset)}
    } else {
        pr_info!("maybe a thread");        
        unsafe{(*thread::RROS_TRHEAD_FACTORY.locked_data().get()).build.unwrap()(&mut thread::RROS_TRHEAD_FACTORY, cstr_u_name, Some(u_attrs), 0, &state_offset)}
    };
...
// }
    ...
    unsafe{
        ...
        pr_info!("the val is {}", val.reserved_fd());
        ret |= rust_helper_put_user(val.reserved_fd() as u32, &mut (*(arg as *mut RrosCloneReq)).efd as *mut i32 as *mut u32);
        pr_info!("the ret is {}", ret);
