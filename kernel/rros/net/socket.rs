...
pub trait RrosNetProto {
    ...
    fn oob_send(&self,sock:&mut RrosSocket, msg:*mut UserOobMsghdr, iov_vec :&mut [iovec]) -> isize;
    fn oob_receive(&self,sock:&mut RrosSocket,msg:*mut UserOobMsghdr, iov_vec : &mut [iovec]) -> isize;
...
impl RrosSocket{
    ...
    pub fn charge_socket_wmem(&self,mut skb:NonNull<RrosSkBuff>)->bool{
        ...
        self.wmem_count.fetch_add(unsafe{skb.as_ref().truesize} as i32, Ordering::Relaxed);
        unsafe{skb.as_mut().net_cb_mut().tracker = self as *const _ as *mut RrosSocket;}
    ...
    pub fn charge_socket_wmem_timeout(&mut self,skb:&mut RrosSkBuff,timeout:KtimeT,tmode:rros_tmode) ->i32{
        ...
        let mut wmem_wait = unsafe{
            &mut *(&self.wmem_wait as *const _ as *mut RrosWaitQueue)
        };
    ...
    pub fn send_or_recv(&mut self,msghdr:*mut UserOobMsghdr,cmd:i32)->i32{
        ...
        let mut iov_vec =  unsafe{core::slice::from_raw_parts_mut(iov, iovlen as usize)};
...
pub fn uncharge_socke_wmem(skb:&mut RrosSkBuff){
    ...
    let flags = unsafe{rust_helper_raw_spin_lock_irqsave(&mut rsk.wmem_wait.lock as *const _ as *mut bindings::hard_spinlock_t)};
    ...
    unsafe{rust_helper_raw_spin_unlock_irqrestore(&mut rsk.wmem_wait.lock as *const _ as *mut bindings::hard_spinlock_t, flags);}
...
unsafe extern "C" fn create_rros_socket(net:*mut bindings::net, sock: *mut bindings::socket, protocol:i32, kern:i32) -> i32
{
    unimplemented!();
}
//     // extern "C"{
...
fn sock_oob_attach(sock: *mut bindings::socket) -> i32
{
	 ...
	 * registered in RROS.
	 */
    let proto = find_oob_proto(sk_family!(sk).into(), unsafe{(*sk).sk_type}.into(), be16::new(unsafe{(*sk).sk_protocol}));
    if proto.is_none(){
...
pub fn rros_import_iov(iov_vec:&[bindings::iovec],data:*mut u8,mut len:u64,remainder:Option<&mut usize>)->i32{
    extern "C"{
...
pub fn rros_export_iov(iov_vec:&mut [bindings::iovec],data:*const u8,len:usize)->i32{
    extern "C"{
...
pub static rros_family_ops : NetProtoFamily = NetProtoFamily(bindings::net_proto_family{
    ...
    owner :unsafe {THIS_MODULE.get_ptr()},
});
